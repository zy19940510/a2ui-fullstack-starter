...
type StreamA2UIMessage = v0_8.Types.ServerToClientMessage & {
  __assistantMessageId?: string | null
}
...
export default function Home() {
  ...
  // A2UI Processor
  const processor = useMemo(() => new v0_8.Data.A2uiMessageProcessor(), []);

  // 消息列表容器引用
  const messagesEndRef = useRef<HTMLDivElement>(null);

  // 处理 A2UI 消息
  const handleA2UIMessage = (message: StreamA2UIMessage) => {
    console.log('📦 Received A2UI message:', message);

    // 处理 A2UI 消息
    const { __assistantMessageId: ownerAssistantId, ...a2uiMessage } = message;
    processor.processMessages([a2uiMessage]);

    // 如果是 beginRendering，设置 surfaceId
    const beginSurfaceId = message.beginRendering?.surfaceId;
    if (beginSurfaceId) {
      setCurrentSurfaceId(beginSurfaceId);
    }

    // 如果能拿到这条 a2ui 事件所属的 assistant 消息，则为该轮创建独立 surface 映射
    if (ownerAssistantId && beginSurfaceId) {
      setSurfaceIdByMessageId((prev) => ({
        ...prev,
        [ownerAssistantId]: beginSurfaceId,
      }));
    }
  };

  const { messages, isLoading, sendMessage, stop } = useSSE(
