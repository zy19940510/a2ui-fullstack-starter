...
import type { WeatherAction, WeatherData, WeatherTranslations } from './weather-types'
...
export function A2UIWeather(props: A2UIComponentProps) {
  ...
  if (rawWeatherData instanceof Map) {
    ...
    const valueMap = (rawWeatherData as { valueMap?: unknown }).valueMap
    weatherData = Array.isArray(valueMap)
      ? (valueMapToObject(valueMap) as WeatherData)
      : defaultWeatherData
  ...
  // 3. 获取 actions
  const refreshAction = componentProps.refreshAction as WeatherAction | undefined
  const changeCityAction = componentProps.changeCityAction as WeatherAction | undefined
