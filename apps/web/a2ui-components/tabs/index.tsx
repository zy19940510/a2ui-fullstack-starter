...
import { resolveBoundValue } from '@a2ui-web/a2ui-react-renderer/utils/resolveValue'

type RenderNode = React.ComponentProps<typeof ComponentRenderer>['node']
type TabItem = { title: string; contentNode: RenderNode }
...
export function A2UITabs(props: A2UIComponentProps) {
  ...
  // 解析标签页数组
  const rawItems = componentProps.items as unknown
  const items: TabItem[] = []
  ...
  if (Array.isArray(rawItems)) {
    for (const item of rawItems) {
      ...
      const contentNode = itemRecord.content as RenderNode | undefined
