...
// 处理用户操作
export function handleUserAction(processor: any) {
  return async (message: any) => {
    ...
    switch (action.name) {
      case 'refresh-weather':
        const currentCity = action.context?.city || '上海'
        ...
        processor.processMessages([
          {
            dataModelUpdate: {
              ...
              contents: [
                ...
                { key: 'weatherCode', valueNumber: newWeather.weatherCode },
      ...
      case 'change-city':
        console.log('[Weather] Change city from:', action.context?.currentCity)

        // 切换到预设的城市列表
        const cities = ['上海', '北京', '广州', '深圳', '杭州', '成都']
        const currentIndex = cities.indexOf(action.context?.currentCity || '')
        ...
        processor.processMessages([
          {
            dataModelUpdate: {
              ...
              contents: [
                ...
                { key: 'weatherCode', valueNumber: cityWeather.weatherCode },
